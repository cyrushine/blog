...
平台：WeTest - 兼容性测试 - 选取 Android 5.1 至 Android 12 共 8 个机型
...
测试流程：APP 启动后 5s 初始化 xCrash ANR，3s 后启动 AnrService，通过启动 Service 超时来触发 ANR
日志分析：将日志文件逐个下载下载，放到 LogPad 上过滤出 tag 为 xcrash_dumper 的条目
...
但是 AnrService 运行 30s 未返回并没有触发 ANR，而后通过手动点击阻塞主线程 60s 才触发 ANR

没有继续深入测试触发 ANR 的阈值，总之 SIGQUIT 处理器还是能捕获到 ANR 的，从收到 SIGQUIT 到写入调用栈重新抛出 SIGQUIT 用时 0.8s

系统的 ANR 处理流程会将日志写入到 /data/anr/traces.txt
