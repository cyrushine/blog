...
![inject.jpg](../../../../image/2021-03-12-how-tinker-work/inject.jpg)
...
`Application.mBase(ContextImpl).mPackageInfo(LoadedApk).mClassLoader` 在实例化 `Activity` 时会用到，在[这篇文章](../../../../2020/11/03/launch-activity-sequence/)里可以找到入口点：
...
另一块是针对局部 `Context` 的，比如 `Activity`，从[Activity 启动流程时序图](../../../../2020/11/03/launch-activity-sequence/)可以找到创建 `Activity` 的入口点
