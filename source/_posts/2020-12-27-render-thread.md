...
接着[上一篇文章](../../../../2020/12/13/ui-thread-in-vsync/)，在上篇文章里我们知道了 ui thread 在 view drawing 阶段产生了 `DisplayList`，而 render thread 会根据 `DisplayList` 执行真正的渲染工作，主要是 `DrawFrameTask.syncFrameState` 和 `CanvasContext.draw` 这两个方法
...
还记得在[上篇文章](../../../../2020/12/13/ui-thread-in-vsync/)里提到， DisplayList 在 endRecording 阶段被放在 `RenderNode.mStagingDisplayList`，这时候转移到 `mDisplayList`
